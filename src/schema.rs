...
// Generated by build.rs: `schema/cybergrid.json` (metadata) merged with
// every theme file under `schema/themes/**/*.json`. The source stays
// chunked — one palette per file — this is the one place the merged whole
// gets embedded into the binary.
const RAW: &str = include_str!(concat!(env!("OUT_DIR"), "/cybergrid_merged.json"));
